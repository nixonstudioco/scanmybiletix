...
class PrinterDetectionService {
  ...
  private async checkSystemPrinters(): Promise<PrinterDetectionResult> {
    try {
      // Avoid circular dependency by checking printers directly
      // Instead of calling printerService.getAvailablePrinters() which imports this service
      // We'll just check if we have any cached printers or return not available
      return this.createNoConnectionResult('System printer check not available');
    } catch (error) {
